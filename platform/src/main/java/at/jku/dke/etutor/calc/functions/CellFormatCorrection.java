...
public class CellFormatCorrection extends CorrectnessRule {
    ...
    public Feedback checkCorrectness(XSSFWorkbook solution, XSSFWorkbook submission) throws Exception {
        if (!checkCorrectCellFormat(solution, submission)) {
            return new Feedback(false, "Your cells are not in the correct Format, are not correct hidden or not correct locked!");
        }
     ...
     * @return true if the solution has the same formatted cells as the submission (format, hidden, locked, usw.)
     * for more information check the functions above
     */
    public static boolean checkCorrectCellFormat (XSSFWorkbook solution, XSSFWorkbook submission) throws Exception {
            ...
            for (Row row : sheetSolution) {
                for (Cell cell : row) {
                    ...
                    if (FillColorHex.isCheckCellFormatCell(sheetSolution, cell) || FillColorHex.isCalculationCell(sheetSolution, cell) || FillColorHex.isValueCell(sheetSolution, cell) || FillColorHex.isDropdownCell(sheetSolution, cell)) {
                        ...
                        if (!checkDataFormat(cellSolution, cellSubmission) || !checkExactlyMatchingCellFormat(cellSolution, cellSubmission) || !checkHidden(cellSolution, cellSubmission) || !checkLocked(cellSolution, cellSubmission)) {
                            return false;
                        }
                    }
                }
