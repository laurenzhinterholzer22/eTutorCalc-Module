...
public class ChartOptions extends CorrectnessRule {
    ...
    public Feedback checkCorrectness (XSSFWorkbook solution, XSSFWorkbook submission) throws Exception {
        if (!correctChartOptions(solution, submission)) {
            return new Feedback(false, "The settings of your Charts are not correct!");
        }
     ...
     * @return true if the chart of the submission are the same as the charts of the solution regarding type, range, ....
     * for more information check the functions above
     */
    public static boolean correctChartOptions (XSSFWorkbook solution, XSSFWorkbook submission) {
        return correctChartType(solution, submission) && correctChartRange(solution, submission) && checkHasTitle(solution, submission) && checkNumberOfAxis(solution, submission);
    }
