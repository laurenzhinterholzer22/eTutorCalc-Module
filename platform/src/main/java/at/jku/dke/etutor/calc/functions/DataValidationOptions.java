...
public class DataValidationOptions extends CorrectnessRule {
    ...
    public Feedback checkCorrectness(XSSFWorkbook solution, XSSFWorkbook submission) throws Exception {
        if (!correctDataValidationOptions(solution, submission)) {
            return new Feedback(false, "Your Data Validations are not correct!");
        }
     ...
     * @return true if the dataValidation (DataValidationConstraint, ErrorBox and PromptBox) of the solution and submission is the same
     */
    public static boolean correctDataValidationOptions (XSSFWorkbook solution, XSSFWorkbook submission) {
        for (int i = 0; i < solution.getNumberOfSheets(); i++) {
            ...
            if (cellRangeListSolution.size() != cellRangeListSubmission.size()) {
                ...
                for (XSSFDataValidation dataValidationSolution : dataValidationListSolution) {
                    ...
                    if (!correctDataValidationConstraint(dataValidationSolution, dataValidationSubmission) || !correctDataValidationErrorBox(dataValidationSolution, dataValidationSubmission) || !correctDataValidationPromptBox(dataValidationSolution, dataValidationSubmission)) {
                        return false;
                    }
                }
            }
