...
public class PivotTableOptions extends CorrectnessRule {
    ...
    public Feedback checkCorrectness(XSSFWorkbook solution, XSSFWorkbook submission) throws Exception {
        if (!correctPivotTableOptions(solution, submission)) {
            return new Feedback(false, "The settings of your Pivot Tables are not correct!");
        }
     ...
     * @return true if the solution has the same pivot table settings as the submission
     * for more information check the functions above
     */
    public static boolean correctPivotTableOptions (XSSFWorkbook solution, XSSFWorkbook submission) {
        return  correctPivotTableDataFieldNames(solution, submission) && correctPivotTableAreaReference(solution, submission);
    }
