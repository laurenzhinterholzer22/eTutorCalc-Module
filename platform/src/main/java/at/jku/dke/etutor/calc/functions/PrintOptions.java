...
public class PrintOptions extends CorrectnessRule {
    ...
    public Feedback checkCorrectness(XSSFWorkbook solution, XSSFWorkbook submission) throws Exception {
        if (!correctPrintOptions(solution, submission)) {
            return new Feedback(false, "The Print Options of your submission is are not correct!");
        }
     ...
     * @return true if the print options of the solution and submission are the same (regarding the functions above)
     */
    public static boolean correctPrintOptions (XSSFWorkbook solution, XSSFWorkbook submission) {
        return correctPrintArea(solution, submission) && correctRepeatingRows(solution, submission) && correctRepeatingColumns(solution, submission);
    }
        ...
        for (int i = 0; i < numberOfSheetsSolution; i++) {
            if (solution.getPrintArea(i) != null) {
                if (submission.getPrintArea(i) != null) {
                    ...
                    // TODO: should the sheetname also be evaluated (fails if the sheetname is renamed)
